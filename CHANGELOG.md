...
- **A "Handing evidence to someone else" README section**, documenting the zip-free path the `ext-zip` guard points at: copy the store directory (or read raw canonical bytes via `RawChainStore`) and verify with `attest verify --storage-root` or `StaticVerifier`. Includes what a directory handoff gives up versus a bundle, and the note that a *truncated* copy still verifies — a shorter chain is still internally consistent, so completeness is established from the stated range, which is one of the things a bundle manifest carries. `DirectoryHandoffTest` pins all three behaviours.
